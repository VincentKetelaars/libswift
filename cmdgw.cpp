...
void CmdGwGotPEX(SwarmID &swarmid, bool enable)
{
	fprintf(stderr, "PEX %s %d\n", swarmid.hex().c_str(), enable);
	cmd_gw_t* req = CmdGwFindRequestBySwarmID(swarmid);
	if (req == NULL)
		return;
...
//        }
		...
		if (peer == Address() && saddr == Address()) // You have to have one of these
			return ERROR_MISS_ARG;
        CmdGwGotPEERADDR(swarmid,peer,saddr);
